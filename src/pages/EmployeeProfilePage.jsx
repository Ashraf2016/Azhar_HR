...
const EmployeeProfilePage = () => {
  ...
  // Format date helper function - moved to the top so it's defined before use
  ...
  const generatePDF = async () => {
    ...
    try {
      ...
      await generatePDFWrapper(
        MyDocument,
        pdfData,
        `${data.name || "employee"}-career-${new Date()
          .toISOString()
          .replace(/[:.]/g, "-")}.pdf`
      );
    } catch (error) {
  ...
  const generateIaratPDF = async () => {
    ...
    try {
      ...
      await generatePDFWrapper(
        IaraatDocument,
        pdfData,
        `${data.name || "employee"}-iaraat-${new Date()
          .toISOString()
          .replace(/[:.]/g, "-")}.pdf`
      );
    } catch (error) {
  ...
  const generatePunPDF = async () => {
    ...
    try {
      ...
      const pdfData = {
        punishments: data.map((item, index) => ({
          ...
          execution_order_date:
            formatDate(item.execution_order_date) || "غير محدد",
          execution_order_date_raw: item.execution_order_date || "غير محدد",
      ...
      await generatePDFWrapper(
        PunishmentsDocument,
        pdfData,
        `${data.name || "employee"}-punishments-${new Date()
          .toISOString()
          .replace(/[:.]/g, "-")}.pdf`
      );
  ...
  const generateEgazatPDF = async () => {
    ...
    try {
      ...
      await generatePDFWrapper(
        EjazatDocument,
        pdfData,
        `${data.name || "employee"}-Ejazat-${new Date()
          .toISOString()
          .replace(/[:.]/g, "-")}.pdf`
      );
      // const blob = await pdf(
  ...
  if (!employee)
    return <div className="text-center py-12">لم يتم العثور على الموظف</div>;
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <div className="lg:col-span-3">
              ...
              <div className="space-y-4">
                ...
                <div className="flex items-center">
                  ...
                  <div>
                    ...
                    <p className="text-gray-900">
                      {employee.fileNumber || employeeId || "غير محدد"}
                    </p>
                  </div>
                </div>
                <div className="flex items-center">
          ...
          {employee.careerProgression &&
            employee.careerProgression.length > 0 && (
              <div className="mt-8 bg-white rounded-lg shadow-md p-6">
                <h2 className="text-xl font-semibold text-gray-900 mb-4">
                  التدرج الوظيفي
                </h2>
                  ...
                  <table className="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                      {[...employee.careerProgression]
                        .sort(
                          (a, b) =>
                            new Date(b.dateOfOccupation) -
                            new Date(a.dateOfOccupation)
                        )
                        .map((career, index) => (
            ...
            <div className="mt-8 bg-white rounded-lg shadow-md p-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                  <p className="text-sm text-gray-500">المنصب</p>
                  <p className="text-gray-900">
                    {employee.previousPosition.title}
                  </p>
                </div>
                <div>
                ...
                <div>
                  ...
                  <p className="text-gray-900">
                    {formatDate(employee.previousPosition.endDate)}
                  </p>
                </div>
              </div>
            </div>
