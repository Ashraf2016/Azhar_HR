...
//       // تحديث العدد الكلي الثابت من API أول مرة فقط أو جلبه من التخزين المحلي
//       if (fixedTotal === 0 && storedTotal) {
//         setFixedTotal(parseInt(storedTotal));
//       } else if (fixedTotal === 0 && noFilters && res.pagination?.totalItems > 0) {
//         setFixedTotal(res.pagination.totalItems);
//         localStorage.setItem("total", res.pagination.totalItems.toString());
//       }
//     } catch (err) {
//       console.error(err);
//       setEmployees([]);
...
      // هذا يمثل عدد الموظفين المطابقين للتصفية
...
        !ufnQuery.trim();
      
      // 2. منطق تحديث العدد الكلي الثابت:
      // نحدّث fixedTotal إذا كنا نجلب كل البيانات (لا يوجد فلاتر) 
      // وهذا هو المكان الوحيد الذي يكون فيه totalItems مساوياً للإجمالي الكلي.
      if (noFilters && res.pagination?.totalItems > 0) {
        setFixedTotal(res.pagination.totalItems);
        localStorage.setItem("total", res.pagination.totalItems.toString());
      } 
      
      // 3. إذا كان fixedTotal لم يتم تحديثه بعد، جلبه من التخزين المحلي (في كل الأحوال)
      const storedTotal = localStorage.getItem("total");
      if (fixedTotal === 0 && storedTotal) {
        setFixedTotal(parseInt(storedTotal));
      }
