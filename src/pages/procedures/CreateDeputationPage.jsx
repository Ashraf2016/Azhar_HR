...
const AddDeputationPage = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    status: passedData?.status || "pending",
  ...
  useEffect(() => {
    ...
    // إذا لم يكن هناك توكن في localStorage، لا نستدعي ال API
    // هذا يمنع الـ interceptor في axios من تنفيذ `navigate('/')`
    // عند تلقي 401 لمستخدم لم يقم بتسجيل الدخول بعد.
  ...
  const handleSubmit = async (e) => {
    ...
    // تحضير البيانات - استبدال university_file_number بـ employee_id للإضافة الجديدة
    const requestData = isEditMode 
      ? formData 
      : { ...formData, employee_id: employeeID };
    
    // حذف university_file_number من البيانات المرسلة للإضافة الجديدة
    if (!isEditMode) {
      delete requestData.university_file_number;
    }
    ...
    try {
      ...
      if (isEditMode) {
            ...
            setTimeout(() => navigate(`/deputation/${employeeID}`), 1000);
            return;
      }
...
                <label className="font-medium text-gray-700 mb-1">نوع الإعارة</label>
        <CustomDropdown
          options={[{ value: "", label: "اختر نوع الإعارة" }, ...types.map((t) => ({ value: t, label: t }))]}
          value={formData.type || ""}
          onChange={(val) => setFormData(prev => ({ ...prev, type: val }))}
          placeholder="اختر نوع الإعارة"
        />
            </div>
